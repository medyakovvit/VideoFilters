...
ThresholdRunnable::ThresholdRunnable(ThresholdVideoFilter *filter) :
    pFilter(filter)
{

}

ThresholdRunnable::~ThresholdRunnable()
{
    qDebug() << "ThresholdRunnable d-tor";
}
...
QVideoFrame ThresholdRunnable::run(QVideoFrame *input, const QVideoSurfaceFormat &surfaceFormat, QVideoFilterRunnable::RunFlags flags)
{
    ...
    cv::Mat thresholded;
    cv::threshold(inMat, thresholded, pFilter->threshold(), 255.0, cv::THRESH_BINARY);
